...
fn parse_i16_as_u8(arg: i16) -> Result<u8, String> {
    if arg >= 0 && arg <= (u8::MAX as i16) {
        return Ok(arg as u8);
    } else {
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let red = parse_i16_as_u8(tuple.0)?;
        let green = parse_i16_as_u8(tuple.1)?;
        let blue = parse_i16_as_u8(tuple.2)?;
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            return Err("Slice is too small or too big".to_owned());
        }
